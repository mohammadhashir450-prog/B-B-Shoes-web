...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Welcome to the Insider. Redirecting to vault...');
      setTimeout(() => {
        router.push('/login');
      }, 2000);
    } catch (error: any) {
  ...
  return (
    <div className="min-h-screen bg-[#0B101E] relative flex flex-col items-center justify-center overflow-x-hidden selection:bg-[#D4AF37]/30 selection:text-white">
      ...
      <motion.div 
        ...
        className="w-full max-w-[460px] relative z-10 px-6 pt-32 pb-24"
      >
            ...
            {success && (
              ...
              >
                <div className="p-4 bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-2xl flex items-center justify-center">
                  <p className="text-[#D4AF37] text-xs tracking-wide text-center font-bold">{success}</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
